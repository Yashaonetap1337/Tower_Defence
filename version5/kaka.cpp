...
// Game implementation
Game::Game() : map(120, 60), currentWave(0), cursorX(0), cursorY(map.getHeight()/2) {}
...
void Game::render() {
    ...
    for (const auto& point : map.getPath()) {
        mvaddch(point.second, point.first, '#' | COLOR_PAIR(1)); // Добавим цвет для пути
    }
    
    // Отрисовка башен
    for (auto& tower : towers) {
        mvaddch(tower->getY(), tower->getX(), 'T' | A_BOLD);
    }
    
    // Отрисовка врагов
    for (auto& enemy : enemies) {
        if (enemy->isAlive()) {
            mvaddch(enemy->getY(), enemy->getX(), 'E' | COLOR_PAIR(2));
        }
    }
