...
struct WaveInfo {
    int totalEnemies;
    float spawnInterval;
    std::vector<std::pair<EnemyType, int>> enemyTypes; // Тип и здоровье
};
...
public:
    ...
    bool isWaveInProgress() const { return isSpawning || !enemies.empty(); }
};

struct Projectile {
    int startX, startY;   // Позиция башни
    int currentX, currentY;// Текущая позиция
    Enemy* target;        // Цель
    float progress;        // Прогресс движения (0.0 - 1.0)
    int damage;            // Урон
    
    Projectile(int sx, int sy, Enemy* t, int dmg) 
        : startX(sx), startY(sy), target(t), progress(0.0f), damage(dmg) {
        currentX = sx;
        currentY = sy;
    }
};
...
public:
    ...
    virtual void attack(Enemy& enemy, std::vector<Projectile>& projectiles);
...
public:
    ...
    const float projectileSpeed = 0.1f; 
    ~Game();
